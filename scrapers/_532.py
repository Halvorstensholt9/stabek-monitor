"""
532.no – norsk nettbutikk for originale fotballdrakter (Shopify).
Samme mønster som Forza/Classickits: products.json + 15-min cache.
"""
...
class S532Scraper:
    ...
    def search(self, keyword: str) -> List[Dict]:
        ...
        for p in _product_cache:
            ...
            if ad:
                matches.append(ad)
        logger.info("532.no '%s': %d treff", keyword, len(matches))
        return matches
    ...
    def _fetch_all(self) -> List[dict]:
        ...
        for page in range(1, 20):
            try:
                ...
                logger.debug("532.no products.json feil side %d: %s", page, exc)
                break
            if not prods: break
            all_products.extend(prods)
            if len(prods) < 250: break
        logger.info("532.no lager oppdatert: %d produkter", len(all_products))
        return all_products
